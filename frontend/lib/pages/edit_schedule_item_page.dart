...
class _EditScheduleItemPageState extends State<EditScheduleItemPage> {
  ...
  late final TextEditingController locationQueryController;
  ...
  void initState() {
    ...
    locationQueryController = TextEditingController(
      text: widget.item.locationAddress ?? '',
    );

    selectedDay = DateTime.parse(widget.item.day);
    startTime =
        parseTime(widget.item.startTime) ??
        const TimeOfDay(hour: 10, minute: 0);
    endTime =
        parseTime(widget.item.endTime) ?? const TimeOfDay(hour: 11, minute: 0);
    selectedType = widget.item.type;

    selectedLat = widget.item.lat;
    selectedLng = widget.item.lng;

    locationQueryController.addListener(handleLocationTextChanged);

    WidgetsBinding.instance.addPostFrameCallback((_) {
      preloadLocationTextFromCoordinates();
    });
  }
  ...
  Future<void> preloadLocationTextFromCoordinates() async {
    ...
    try {
      await ScheduleService.updateScheduleItem(
        ...
        clearLocation:
            selectedLat == null &&
            selectedLng == null &&
            locationQueryController.text.trim().isEmpty,
      );
  ...
  Future<void> handleDelete() async {
    ...
    try {
      await ScheduleService.deleteScheduleItem(
        tripId: widget.tripId,
        itemId: widget.item.id,
      );
